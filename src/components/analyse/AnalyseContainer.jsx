...
class AnalyseContainer extends Component {
    ...
    render() {
        const {user, userId, posts, photos, estimation, errors, progressId} = this.state;
        const settings = {posts, photos};
        if (progressId) {
            return <Redirect to={'/progress'}/>
        }
        return (
            <AnalyseView onPhotosChange={this.handlePhotosChange} onPostsChange={this.handlePostsChange}
                         onUserIdChange={this.handleUserIdChange} settings={settings} userId={userId} user={user}
                         errors={errors} estimation={estimation} onAnalyse={this.handleAnalyse}/>
        );
    }
}
